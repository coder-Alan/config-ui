 ...
 * 	@field {String} label-key 标签字段名
 * 	@field {String} value-key 值字段名
 * 	@field {String} border-align = [top|bottom|vertical] 边框位置
 * 		@value top 上边框
 * 		@value bottom 下边框
 * 		@value vertical 上下边框
 * 		@default bottom
 * 	@field {String} label-size 标签字体大小
 * 	@field {String} label-color 标签颜色
 * 	@field {String} label-style 自定义标签的样式
 * 	@field {String} label-icon 标签尾部图标
 * 	@field {String} label-icon-style 标签尾部图标样式
 * 	@field {String} layout = [horizontal|vertical] 标签布局方式
 * 		@value horizontal 水平方式
 * 		@value vertical 垂直方式
 * 		@default horizontal
 * 	@field {String} custom-style 自定义样式
 * 	@field {Boolean} disabled 是否禁用
 * @event {Function} click CoFormLabel组件图标的点击事件
 */
